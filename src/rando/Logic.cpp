// Ported from SoH logic.cpp — clean-room build for MultiShip (includes repointed to shims).
#include "rando/Logic.h"
#include "rando/Context.h"
#include "rando/Graph.h"
#include "rando/GameShim.h"
#include <string>
#include <vector>
#include <map>
#include <array>
...
#endif
#ifndef StartPerformanceTimer
#define StartPerformanceTimer(x) ((void)0)
#define StopPerformanceTimer(x) ((void)0)
#endif
namespace Rando {
...
bool Logic::ShopItemNotForSale(RandomizerCheck loc) {
    // Shopsanity / shop shield-tunic gating is off under default settings.
    return false;
}
...
uint8_t Logic::GetSmallKeyCount(uint32_t dungeonIndex) {
    // Generation has no playthrough scene-flag state, so "used keys" is always 0;
    // logical key count == collected keys.
    int8_t dungeonKeys = mSaveContext->inventory.dungeonKeys[dungeonIndex];
    return dungeonKeys == -1 ? 0 : static_cast<uint8_t>(dungeonKeys);
}
